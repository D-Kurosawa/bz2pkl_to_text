"""Measure processing time module"""
import time
...
def app_time(func):
    """Decorator to application processing time"""
...
def func_time(func):
    """Decorator to function processing time"""
    ...
    def wrapper(*args, **kwargs):
        ...
        e = time.time() - s
        print(f"\n{'-' * 60}")
        print(f"function name : {func.__name__} : {e:.10f} [sec]")
        print(f"{'-' * 60}\n")
        return result
